...
ClientRouter.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!userD) {
      ...
      return;
    } else if (userD.password !== password) {
...
ClientRouter.get("/getupdates", async (req, res) => {
  try {
    if (
      !(await verifyToken(req, async (userName) => {
        ...
        if (user) {
          ...
          res.status(500).send("error try update uer info");
        }
      }))
    ) {
    ...
    console.log("error try get updates:", e);
  }
});

ClientRouter.post("/joingroup", async (req, res) => {
  //!!!!!!!add operation to resolve token!!!!!!
  ...
  try {
    ...
    console.log("error while trying add to group:", err);
  }
});
...
ClientRouter.post("/submitjoin", async (req, res) => {
  ...
  try {
    ...
    if (!group || !client) {
      res.status(500).send("cant find sender or client");
      return;
    }
    ...
    console.log("error try submit request:", e);
  }
});
...
ClientRouter.delete("/leavegroup", async (req, res) => {
  ...
  try {
    ...
    if (!user || !sender) {
      res.status(500).send("error find user or sender");
      return;
    }
    if (!sender.group.includes(userId)) {
    ...
    console.log("error try leave group", e);
  }
});
...
async function cancelrequest(req, res, userId, groupId) {
  try {
    ...
    if (request.length === 0) {
      res.status(500).send("not request found");
      return;
    }
    await sender.updateOne({ $pull: { requests: { userId: userId } } });

    await user.updateOne({ $pull: { requests: { userId: groupId } } });
  ...
  try {
    if (
      !(await verifyToken(req, async (userName) => {
        ...
        const missionsToSend = missions.filter((mission,i) => {
          if (
            mission.type === "public" ||
            (mission.type === "privet" &&
              user.group.includes(mission.sender) &&
              !mission.blockedUsers.includes(userName))
          ) {
            if (!mission.saved || mission.saved === userName) {
              return mission;
            }
          }
        });
...
ClientRouter.delete("/deletetaskhistory", async (req, res) => {
  ...
  try {
    if (
      !(await verifyToken(req, async (userame) => {
